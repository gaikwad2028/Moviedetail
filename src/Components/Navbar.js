...
export default function Searchbar(props) {
    ...
    return (
        <>
            ...
            {searchlabel.length > 1 ? <div className='Search_hero_container'>  {
                Search !== undefined && Search.results !== undefined && Search.results.length > 0 && <div className='search1'>
                    {
                        Search.results.map((res, index) => {
                            return <div className='search_sub' key={index}><NavLink to={`/Singelmoviepage/${res.id}`} onClick={header}><Card Titel={res.original_title.toUpperCase()} Image1={"https://image.tmdb.org/t/p/w500" + res.backdrop_path} Rating={res.vote_average}></Card></NavLink></div>
                        })
                    }
                </div>
